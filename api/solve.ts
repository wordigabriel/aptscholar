...
export default async function handler(req: any, res: any) {
  // Set CORS headers so that client can query it directly if needed
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(500).json({
        error: "GEMINI_API_KEY environment variable is not configured. Please add it to Vercel Environment Variables or your server environment.",
      });
    }
